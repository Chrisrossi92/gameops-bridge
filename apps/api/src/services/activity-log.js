import { activityLogItemSchema } from '@gameops/shared';
...
function getRawConfidence(event) {
...
function findClosedSessionForEvent(event, closedSessions) {
    const playerName = normalizePlayerKey(event.playerName ?? '');
    return closedSessions.find((session) => (session.endedAt === event.occurredAt
        && (!playerName || normalizePlayerKey(session.playerName) === playerName))) ?? null;
}
...
function summarizeHealthWarning(event) {
...
function buildActivityItem(event, closedSessions) {
    ...
    if (event.eventType === 'PLAYER_LEAVE') {
        ...
        const durationText = formatDuration(typeof event.raw?.sessionDurationSeconds === 'number'
...
export function getActivityLogForServer(serverId, limit = 20) {
