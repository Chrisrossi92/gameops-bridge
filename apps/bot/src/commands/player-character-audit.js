...
function formatList(values, max = 5) {
    if (values.length === 0) {
        return 'None';
    }
...
export const playerCharacterAuditCommand = {
    ...
    async execute(interaction) {
        ...
        const response = await fetch(`${botConfig.apiBaseUrl}/servers/${serverId}/players/known/${encodeURIComponent(playerName)}/audit`);
        if (!response.ok) {
            await interaction.reply({
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Distinct Character IDs', value: String(audit.distinctCharacterIds.length), inline: true }, { name: 'Distinct Platform IDs', value: String(audit.distinctPlatformIds.length), inline: true }, { name: 'Distinct PlayFab IDs', value: String(audit.distinctPlayFabIds.length), inline: true }, { name: 'Character IDs', value: formatList(audit.distinctCharacterIds, 8), inline: false }, { name: 'Platform IDs', value: formatList(audit.distinctPlatformIds, 5), inline: true }, { name: 'PlayFab IDs', value: formatList(audit.distinctPlayFabIds, 5), inline: true }, {
...
//# sourceMappingURL=player-character-audit.js.map