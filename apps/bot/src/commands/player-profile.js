...
function formatRecentSessionLine(endedAt, durationSeconds) {
    return `• ${formatTimestamp(endedAt)} (${formatDurationCompact(durationSeconds)})`;
}
export const playerProfileCommand = {
    ...
    async execute(interaction) {
        ...
        const response = await fetch(`${botConfig.apiBaseUrl}/servers/${serverId}/players/known/${encodeURIComponent(playerName)}`);
        if (!response.ok) {
            await interaction.reply({
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Platform IDs', value: formatList(player.knownPlatformIds), inline: true }, { name: 'PlayFab IDs', value: formatList(player.knownPlayFabIds), inline: true }, { name: 'Character IDs', value: formatList(player.knownCharacterIds), inline: true }, { name: 'Identity Sources', value: formatList(player.identitySources), inline: false })
            .addFields({
            name: 'Recent Sessions',
            value: recentSessions.length > 0
                ? recentSessions
                    .slice(0, 5)
                    .map((session) => formatRecentSessionLine(session.endedAt ?? session.startedAt, session.durationSeconds ?? 0))
                    .join('\n')
                : 'None yet',
            inline: false
        })
